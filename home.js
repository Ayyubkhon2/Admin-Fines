...
// --------------- Variant Switcher ---------------
document.addEventListener("DOMContentLoaded", () => {
  const buttons = document.querySelectorAll(".map__switcher-btn");
  const variants = document.querySelectorAll(".map__variant");

  variants.forEach((v) => {
    v.style.display = v.classList.contains("map__variant--active")
      ? "block"
      : "none";
  });
  ...
  buttons.forEach((button, index) => {
    button.addEventListener("click", () => {
      buttons.forEach((b) => b.classList.remove("map__switcher-btn--active"));
      button.classList.add("map__switcher-btn--active");

      variants.forEach((v, i) => {
        v.classList.remove("map__variant--active");
        v.style.display = i === index ? "block" : "none";
      });

      variants[index].classList.add("map__variant--active");
    });
  });
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  provinces.forEach((province) => {
    ...
    province.addEventListener("mouseenter", () => {
      ...
      tooltip.innerHTML = `
    <div class="map__tooltip-header">${translatedName}</div>
    <div class="map__tooltip-stats">
      <div class="map__tooltip-stat">
        <span class="map__tooltip-icon"><img src="assets/institution-icon.png"></span>
        <span class="map__tooltip-number1">${stat1}</span>
      </div>
      <div class="map__tooltip-stat">
        <span class="map__tooltip-icon"><img src="assets/fines-icon.png"></span>
        <span class="map__tooltip-number2">${stat2}</span>
      </div>
      <div class="map__tooltip-stat">
        <span class="map__tooltip-icon"><img src="assets/amount-icon.png"></span>
        <span class="map__tooltip-number3">${stat3}</span>  
      </div>
    </div>
  `;
...
function initSlider() {
  ...
  function goRight() {
    ...
    const visibleCards = Math.floor(
      track.parentElement.getBoundingClientRect().width / cardWidth
    );
    ...
    resources: {
      ru: {
        translation: {
          ...
          legend1: "- Количество АО",
          legend2: "- Количество штрафов",
          legend3: "- Общая сумма (в млн. сум)",
        },
      },
      en: {
        translation: {
          ...
          hero_text:
            "Automated System for the Application of Enforcement Measures in the Capital Market",
      ...
      uz: {
        translation: {
          ...
          service: "Xizmatlar",
          fine: "Ma'muriy jarimalar to'g'risida ma'lumot olish",
          ...
          legend1: "- AJlar soni",
          legend2: "- Jarimalar soni",
          legend3: "- Umumiy summa (mln. so'mda)",
        },
      },
    },
  },
